...
void trim_leading_spaces(std::string_view& out_line) noexcept {
	while (!out_line.empty() && std::isspace(out_line.at(0))) { out_line = out_line.substr(1); }
}

bool line_comment_at_start(locc::comment_info const& info, std::string_view sub_line) noexcept {
	for (auto& str : info.comment_lines) {
		if (auto const search = sub_line.find(str); search == 0) { return true; }
	}
	return false;
}
...
void count_lines(locc::file_t& out_file) {
	if (auto file = std::ifstream(stdfs::absolute(out_file.path), std::ios::in)) {
		...
		while (std::getline(file, line)) { count_line(info, out_file, open_block, line); }
	}
}
} // namespace
...
async_counter::async_counter(result_t* result) : m_result(result) {
	...
	if (thread_count > 0) {
		...
		for (int count = thread_count; count > 0; --count) { m_workers.push_back(worker(&m_queue)); }
	}
}
...
async_counter::~async_counter() {
	...
	for (auto& task : residue) { task(); }
	locc::log_if(cfg::test(cfg::flag::debug), "\n");
	for (auto& [_, data] : m_result->dist) { data.ratio.divide(data.counts.lines, m_result->totals); }
}
