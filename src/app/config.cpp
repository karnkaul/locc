...
std::optional<locc::file_t> settings::include_file(stdfs::path const& path) const {
	...
	if (filenames_as_ext.find(filename) != filenames_as_ext.end()) {
		return locc::file_t{.path = path, .ext = filename, .id = get_id(filename), .lines = {}};
	}
	auto const ext = path.extension().generic_string();
	if (ext.empty()) {
		return std::nullopt;
	}
	if (std::none_of(ext_passlist.begin(), ext_passlist.end(), [&ext](auto skip) { return skip == ext; })) {
		return std::nullopt;
	}
